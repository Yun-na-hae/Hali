...
BOOL CArrow::OnInitDialog()
{
	...
	if (((CHalliGalli_dlgDlg *)GetParent())->m_level == 1) {//0상  1중   2하
		m_arrowcnt = rand() % 2 + 6; //화살표 개수를 5개~8개까지 랜덤으로 생성하기 위함
		for (int i = 0; i < m_arrowcnt; i++) {
			Arrow_piece* ap = new Arrow_piece(rand() % 2);
			m_arrow_arr.Add(ap);//화살표들을 배열에 추가
		}
	}
	else if (((CHalliGalli_dlgDlg *)GetParent())->m_level == 0) {
		m_arrowcnt = rand() % 4 + 6; //화살표 개수를 5개~8개까지 랜덤으로 생성하기 위함
		for (int i = 0; i < m_arrowcnt; i++) {
